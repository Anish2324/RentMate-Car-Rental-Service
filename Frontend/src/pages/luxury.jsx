...
const Luxury = () => {
  ...
  useEffect(() => {
    axiosInstance.get('/car/get-cars?category=luxury')
  ...
  return (
    <div className="min-h-screen bg-[#181c2f] py-10 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {newCars.length === 0 ? (
          <div className="col-span-3 text-center text-lg text-blue-200">No luxury cars found.</div>
        ) : (
          newCars.map((car) => (
            <div
              key={car._id}
              ...
              <div className="mt-4 flex-1 flex flex-col gap-1">
                ...
                <button
                  className={`mt-4 bg-gradient-to-r from-blue-600 to-pink-600 text-white px-4 py-2 rounded-lg font-bold shadow-lg hover:from-pink-600 hover:to-blue-600 hover:scale-105 transition ${
                    !(car.availability === 'true' || car.availability === true) ? 'opacity-60 cursor-not-allowed' : ''
                  }`}
                  onClick={() => {
                    if (car.availability === 'true' || car.availability === true) {
                      handleBookNow(car);
                    }
                  }}
                  disabled={!(car.availability === 'true' || car.availability === true)}
                >
                  {car.availability === 'true' || car.availability === true
                    ? 'Book Now'
                    : 'Unavailable'}
